                ...
                stage("Packaging Client in Docker Container"){
                    ...
                    steps{
                            ...
                            bat(
                                ...
                                script: "docker run --isolation=process --rm -v \"${WORKSPACE}\\build:c:\\build:rw\" -v \"${WORKSPACE}\\dist:c:\\dist\" -v \"${WORKSPACE}\\scm:c:\\source:ro\" --workdir=\"c:\\TEMP\" --rm %DOCKER_IMAGE_TAG% cmd /C \"mkdir c:\\TEMP\\build && xcopy c:\\build c:\\TEMP\\build cd c:\\TEMP\\build && cpack -G NSIS -G WIX && copy *.msi c:\\dist\\\""
                            )
                    }
                    post{
                        cleanup{
