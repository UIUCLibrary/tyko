                        ...
                        stage("Install Dependencies"){
                            steps{
                                ...
                                bat(
                                    ...
                                    script: 'docker run --isolation=process --rm -v "%WORKSPACE%\\build:c:\\build" -v "%WORKSPACE%\\scm:c:\\source:ro" --workdir="c:\\build" %DOCKER_IMAGE_TAG% conan install c:\\source'
                                    )
                            }
                        }
                        stage("Configure and Build Client"){
                            steps{
                                bat(
                                    ...
                                    script: 'docker run --isolation=process --rm -v "%WORKSPACE%\\build:%WORKSPACE%\\build" -v "%WORKSPACE%\\scm:c:\\source:ro" --workdir="%WORKSPACE%" %DOCKER_IMAGE_TAG% cmake -S c:\\source -B %WORKSPACE%\\build -DCMAKE_TOOLCHAIN_FILE=conan_paths.cmake -DCMAKE_GENERATOR_PLATFORM=x64'
                                )
                                bat(
                                    label: "Running build command from CMake",
                                    script: 'docker run --isolation=process --rm -v "%WORKSPACE%\\build:%WORKSPACE%\\build" -v "%WORKSPACE%\\scm:c:\\source:ro" --workdir="%WORKSPACE%\\build" %DOCKER_IMAGE_TAG% cmake --build %WORKSPACE%\\build --config Release'
                                )
                ...
                stage("Packaging Client in Docker Container"){
                    ...
                    steps{
                            ...
                            bat(
                                ...
                                script: 'docker run --isolation=process --rm -v "%WORKSPACE%\\build:%WORKSPACE%\\build:rw" -v "%WORKSPACE%\\dist:%WORKSPACE%\\dist" -v "%WORKSPACE%\\scm:c:\\source:ro" -v "%WORKSPACE%\\scm\\CI\\shared_docker_scripts:%WORKSPACE%\\scm\\CI\\shared_docker_scripts:ro" --workdir="%WORKSPACE%" %DOCKER_IMAGE_TAG% scm\\CI\\shared_docker_scripts\\package.bat'
                            )
                    }
                    post{
                        cleanup{
