                        ...
                        stage("Backing up database"){
                            steps{
                                script{
                                    ...
                                    sshCommand(
                                        remote: remote,
                                        command: "docker exec  avdatabase_db_1 /bin/bash -c \"mysqldump av_preservation --user='avuser' --password='avpsw' > /tmp/backup.sql\" && ls /tmp/ && docker cp avdatabase_db_1:/tmp/backup.sql ./"
                                        )
//                                    sshCommand(
//                                        remote: remote,
